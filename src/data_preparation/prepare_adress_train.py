...
def list_wavs_with_label(dir_path: Path, label: int) -> List[Tuple[Path, str, int]]:
    ...
    id_str = stem of wav file (e.g., "cc001")
    """
    entries = []
...
def prepare_train_audio_and_labels() -> None:
    ...
    # Collect all wavs from ADReSS cc and cd folders
    ...
    if not all_rows:
        print("[PREP] ERROR: No WAV files found in cc/cd folders. "
              "Check ADRESS_TRAIN_ROOT and that the audio is present.")
        return
