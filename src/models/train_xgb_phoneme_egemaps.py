# src/models/train_xgb_phoneme_egemaps.py
...
from src.config import PHONEME_EGEMAPS_CSV, DATA_DIR
...
def load_phoneme_egemaps() -> pd.DataFrame:
    df = pd.read_csv(PHONEME_EGEMAPS_CSV)
...
def _try_merge_with_subject_id(feat_df: pd.DataFrame, labels_df: pd.DataFrame) -> tuple[pd.DataFrame, str]:
    ...
    raise RuntimeError("No overlap between phoneme_egemaps_diarized.csv and labels_train.csv IDs.")


def prepare_dataset():
    """
    Join phoneme eGeMAPS with subject-level labels.
    ...
    feat_df = load_phoneme_egemaps()

    if feat_df.empty:
        raise RuntimeError(f"{PHONEME_EGEMAPS_CSV} is empty – run phoneme_egemaps extraction first.")

    merged, mapping_name = _try_merge_with_subject_id(feat_df, labels_df)

    print(f"[INFO] Merged {len(merged)} phoneme rows with labels using mapping '{mapping_name}'.")
    ...
    if X.shape[0] == 0:
        raise RuntimeError("After merging, no rows left for training. Check ID formats in labels_train.csv and phoneme_egemaps CSV.")
...
def train_and_tune_xgb_phoneme():
    ...
    print("[INFO] Starting RandomizedSearchCV for phoneme-level XGB...")
    ...
    # Save model + metadata
    out_dir = DATA_DIR / "features_agg" / "phoneme_xgb"
    out_dir.mkdir(parents=True, exist_ok=True)

    model_path = out_dir / "xgb_phoneme_model.joblib"
    params_path = out_dir / "xgb_phoneme_best_params.json"
    feat_path = out_dir / "xgb_phoneme_feature_names.npy"
